...
import eu.girc.economy.actions.ItemInteractAction;
...
public class ExtendedItemStack extends ItemStack implements Listener {
	...
	public static ExtendedItemStack getItem(ItemStack itemStack) {
		for (ExtendedItemStack item : extendedItemStacks) {
			if (itemStack != null && item.getType() == itemStack.getType() && itemStack.hasItemMeta() && itemStack.getItemMeta().getDisplayName().equals(item.getItemMeta().getDisplayName())) {
				return item;
			}
		}
		return null;
